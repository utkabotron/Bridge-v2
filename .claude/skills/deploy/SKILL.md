...
5. ALWAYS restart nginx after `docker compose up -d` — container gets new IP and nginx caches old upstream → 502:
   ```
   ssh bridge "cd /home/deploy/bridge-v2 && docker compose restart nginx"
   ```

6. For wa-service: delete SingletonLock before restart to avoid Chromium lock issues:
   ```
